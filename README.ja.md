...
公式 Spotify Web API で作る、ターミナル向けの高速な Spotify TUI。

spotterm は再生コントロール・検索・ライブラリ閲覧・ライブな「Now Playing」表示（対応端末では
**アルバムのカバーアート**付き）を、すべて 1 つの対話型 TUI で行います。認証は
**Authorization Code + PKCE**（端末に client
...
- **再生コントロール**：再生 / 一時停止 / 次 / 前 / シーク / 音量（TUI から操作）。
- **検索して再生**。
- **ライブラリ閲覧・再生**：プレイリスト / 保存トラック / 保存アルバム。
- **デバイス選択**：Connect デバイス一覧から再生を転送。
- **お気に入り**：現在曲をライブラリに保存 / 解除。

すべての操作は対話型 TUI 内で行います。CLI コマンドは `login` と `tui` の 2 つだけです。
...
- 端末フォントに **[Nerd Font](https://www.nerdfonts.com/)** を設定してください。TUI と `login` の
  出力はアイコン（曲・アーティスト・アルバム・音量・警告など）に Nerd Font グリフを使うため、
  未設定だと豆腐（□）表示になります。
...
spotterm のコマンドは 2 つだけです:

```sh
spotterm login   # 初回認証（PKCE。ブラウザで同意し、トークンをキャッシュ）
spotterm tui     # 対話型の Now Playing ダッシュボードを起動
```

再生・検索・ライブラリ閲覧・デバイス切替など、操作はすべて TUI 内で行います。

> **旧バージョンからの更新時**: ライブラリの **Artists** タブ（フォロー中アーティスト）には
> `user-follow-read` スコープが必要です（ダッシュボード刷新で追加）。既存トークンには含まれないため、
> 一度 `spotterm login` を再実行して許可してください。許可するまで Artists タブは取得エラーを表示します
> （黙って空白にはなりません）。
...
| `s` | 現在曲を保存 / 解除 |
| `/` | 検索して再生 |
| `tab` | ライブラリ / 詳細ペインのフォーカス切替 |
| `[` / `]` | ライブラリ: 前 / 次のタブ（All / Artists / Albums / Playlists / Tracks） |
| `↑` / `↓` | フォーカス中ペインの選択を移動（ライブラリ / 詳細） |
| `enter` | 選択を再生（ライブラリ項目、または詳細ペインのトラック） |
| `d` | デバイス選択（再生を転送） |
| `r` | 更新（再生 / フォーカス中のライブラリタブまたは詳細） |
| `?` | ヘルプ |
| `q` / `Esc` / `Ctrl-C` | 終了 |
