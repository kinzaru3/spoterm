...
- ホスト（mac）で **公式 Spotify アプリが起動・Premium ログイン済み**であること
  （これが再生デバイス＝Connect デバイスになる。**spotifyd は使わない**）。
- 出力先（スピーカー/AirPods 等）は公式アプリ側の設定に従う（spoterm/Web API 側では出力先を制御しない）。
- 以下の手順の `<デバイス名>` は、あなたの公式 Spotify アプリのデバイス名（多くは mac のコンピュータ名）。
  `spoterm devices` で正確な名前を確認できる。
...
```bash
cargo run -q -- devices   # 公式 Spotify アプリを含むデバイス一覧が出る
...
```bash
cargo run -q -- device use <デバイス名>   # 公式 Spotify アプリをアクティブ化（転送）
...
TUI 起動後（TTY 必須）、Now Playing 画面で確認する。表示品質は**端末に依存**する。

- 曲を再生すると Now Playing の**左にカバーアート**が出る。
  - iTerm2 / kitty / WezTerm / Ghostty 等 → **実画像**。
  - Terminal.app 等の非対応端末 → **カラー半ブロック（halfblocks）**で近似表示（画像プロトコル非対応でも必ず何か出る）。
- 曲を切り替えるとアートも切り替わる（曲変化時のみ取得＝体感が軽い）。
- ポッドキャスト等アートが無い/取得前は「**(アートなし)**」プレースホルダが出る（空状態を明示）。
- ネットワークを切って曲送りするとアート取得に失敗し、ステータス行に「⚠ カバーアートの取得に失敗」。
  復帰後に `r` を押すと再取得される（`r` は自動更新再開＋アート再取得を兼ねる）。

期待:
- カバーアートがテキスト（曲名/アーティスト/進捗）と重ならず左列に収まる。
- 画像取得中も UI が固まらない（取得は非同期・5 秒でタイムアウト）。
...
- **再生デバイスは公式 Spotify アプリ**: 音を出すのは mac の公式 Spotify アプリ。出力先（スピーカー/
  AirPods 等）はアプリ側の通常動作に従うため、spotifyd のような「起動時に出力先が固定される・再起動が要る」
  といった制約はない。公式アプリが起動・ログインしていないと `devices` に出ず転送先が無いので注意。
  （旧構成で使っていた spotifyd（librespot ベースの非公式クライアント）はスコープ外。spoterm は librespot 非依存。）
