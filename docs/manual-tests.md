...
- `←`/`→` でタブ切替（**初回のみ取得し、以降はキャッシュ表示**）、`↑`/`↓` で選択、`Enter` で再生。
- `r` で現在タブを再取得（キャッシュを破棄してリロード）。
- `Esc` で Now Playing に戻る。`Ctrl-C` はどの画面でも終了。

期待:
- プレイリスト（先頭50件）・保存トラック/アルバム（各先頭20件）が `title — subtitle` で並ぶ。
- **タブ切替の体感が軽い**：一度開いたタブへ戻るときは再取得せずキャッシュから即表示（ネットワーク待ちが無い）。
- **`r` リロード**：ライブラリを更新した後に `r` を押すと最新の一覧に更新される。
...
- `d` を押す → デバイス選択オーバーレイ。利用可能デバイスが `● / ○` 付きで並ぶ（`●` がアクティブ）。
- `↑`/`↓` で選択、`Enter` で選択デバイスへ転送、`r` で一覧を再取得、`Esc` で Now Playing に戻る。
- `Ctrl-C` はどの画面でも終了。

期待:
- 一覧に `MacBook-spotifyd` を含む利用可能デバイスが表示され、現在アクティブなものに `●` と `(active)`。
- **開くたびに最新取得**：`browse` と違いキャッシュしない（デバイスは起動/終了で出入りするため）。
- `Enter` 転送：選択デバイスがアクティブ化され Now Playing に戻る → デバイス行が転送先に変わる（Connect の
  伝播遅延で 1 テンポ遅れることがある。数秒後に自動ポーリングで反映）。
- 空一覧は「再生可能なデバイスがありません…」、取得失敗・転送失敗は補足行に `…に失敗しました: …`。
- 操作不可（`is_restricted`）や ID の無いデバイスを `Enter` すると転送せずメッセージを表示（silent failure なし）。
- アクティブデバイス無しで `space`/`+`/`-` を押すと「d でデバイスを選択…」と案内される。

### Phase 6.4 — シーク & 現在曲のお気に入り（Seek + save）

**事前に一度だけ再ログインが必要**（保存機能に `user-library-modify` スコープを追加したため）:
```bash
cargo run -q -- login   # ブラウザで再同意 → token.json を更新
```

TUI 起動後（TTY 必須）、Now Playing 画面で操作する。

- `→` / `←` で 5 秒ずつ前後にシーク。**進捗ゲージは即時にジャンプ**する（次ポーリングを待たない）。
- `s` で現在曲をライブラリに保存 / 解除。状態行に `♥ 保存済み` / `♡ 未保存` が出る。

期待:
- シーク: `→` で進み `←` で戻る。曲頭/曲末でクランプ（マイナスや尺超えにならない）。連打も累積する。
- 保存: 初回 `s` で `♥ ライブラリに保存しました`、再度 `s` で `♡ 保存を解除しました`。曲を変えると状態表示が更新される（曲ごとに 1 回だけ保存状態を問い合わせ）。
- トラック情報不明（広告等）で `s` を押すと「保存できません」と案内（silent failure なし）。
- 再ログインしていない場合、`s` は権限エラー（403）で「保存操作に失敗」になる → 上記 `login` を実行。

### Phase 6.5 — UI 仕上げ（ヘルプ / 表示幅 / ステータス）

TUI 起動後（TTY 必須）、Now Playing 画面で操作する。

- `?` を押す → ヘルプオーバーレイに全キー一覧が表示される。**何かキーを押す**（`Esc`/`?`/`q` 等）で戻る。
- フッターが `? ヘルプ   q 終了` に簡略化されている。
- 全角/絵文字を含む長い曲名・アーティストでも行が枠内に収まる（列あふれや折り返し崩れがない）。
- 操作後のステータス（例 `▶ 再生`）は**約 4 秒で自動的に消える**。`⚠` は赤、成功（`▶`/`♥` 等）は緑で表示。
- 通信を切る等で自動更新が止まると、ステータス行に「⚠ 自動更新を停止中です。r で再試行」が**消えずに**表示され続ける（`r` で復帰）。
